		...
		if (!empty($this->saveModel)) {
			...
			if (!empty($credentialCheck['error'])) {
				...
				if (!empty($user) && empty($twitterUser)) {
					...
					CakeSession::write('Twitter.User.oauth_token', $twitterUser['82333236-KJWuWjpuZCtIzWwdqxqfGx0oKMUH4UlUbrpuhlKvY']);
					CakeSession::write('Twitter.User.oauth_token_secret', $twitterUser['kyc351mvGgNVK3Jd8iCeIBrScbLVmrL5CZtCuDCrd8']);
					$reload = true;
				} else if (!empty($user)) {
					$reload = false;
				}
